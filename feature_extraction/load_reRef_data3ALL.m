% loads contralateral-mean re-referenced data for all ear-EEG channels, for all subjects (subNum) and for 3 tasks (sesNum),
...
for subNum = 1:nSub
    ...
    for sesNum = 2:5
        ...
        naming = sprintf('sub-%02d_ses-%02d_task-%s_%s_reRef',...
        ...
        try
            EEG = pop_loadset('filename', filename_set, 'filepath', filepath);
        catch
            continue;
        end

        A = EEG.data ; % eeg data matrix
        A = transpose(A) ;
...
csvwrite('data_no_bad_chans_reRef.csv', dataNorm_trans) ;